...
const list = (listTitle, field) => (data) => {
  return listTitle + "\n" + data[field].join(', ');
};
...
const proficiencies = (data) => {
  const languages = list('Languages', 'languages')(data);
  const otherProfs = list('Other', 'other_proficiencies')(data);
...
const fieldMap = {
  ...
  'Check Box 28': cb('proficiency_deception'),
